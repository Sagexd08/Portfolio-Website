// Perlin noise implementation for realistic brain texture and movement
// Based on improved noise algorithm by Ken Perlin
...
// Generate a 3D noise field for brain texture
  ...
  for (let z = 0; z < depth; z++) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        row.push(fractalNoise(x * scale, y * scale, z * scale));
      }
      slice.push(row);
    }
    field.push(slice);
